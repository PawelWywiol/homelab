...
- `Caddyfile`: Static routing configuration (tracked in git)
- `config/`: Caddy state
- `data/`: Let's Encrypt certificates
- `.env`: Cloudflare credentials (generated by bootstrap.sh)

**Note**: Caddyfile is committed to the repository. Bootstrap.sh only generates the `.env` file with Cloudflare credentials.
