...
load_env() {
    ...
    SEMAPHORE_DATA_DIR="${SEMAPHORE_DATA_DIR:-${HOME}}"
...
setup_semaphore() {
    ...
    local semaphore_dir="${SEMAPHORE_DATA_DIR}/.semaphore"

    # Create data directories
    if [ ! -d "$semaphore_dir" ]; then
        log_info "Creating Semaphore data directory at ${semaphore_dir}..."
        mkdir -p "${semaphore_dir}"/{config,tmp}
        # Semaphore container runs as uid 1001
        sudo chown -R 1001:1001 "${semaphore_dir}"
        sudo chmod -R 755 "${semaphore_dir}"
    else
        log_skip "Semaphore data directory"
    fi

    # Create .env file
    local env_file="${SCRIPT_DIR}/docker/config/semaphore/.env"
    if [ -f "$env_file" ]; then
...
EOF
    ...
    # Generate Caddyfile (imports static template + adds dynamic semaphore/webhook)
...
# Static routes imported from Caddyfile.template
...
*.${BASE_DOMAIN} {
    ...
    # Import static reverse proxy rules
    import /etc/caddy/Caddyfile.template
...
print_summary() {
    ...
    echo "     Then update: ~/docker/config/webhook/.env"
    echo "     Set: SEMAPHORE_API_TOKEN=<your-token>"
    echo "     Restart: make webhook restart"
    echo ""
...
main() {
    echo ""
    echo "============================================================================="
    log_info "x000 Control Node Bootstrap"
    echo "============================================================================="
    echo ""
...
if [ "${1:-}" = "--regenerate-caddy" ]; then
    load_env
    rm -f "${SCRIPT_DIR}/docker/config/caddy/Caddyfile"
    setup_caddy
    log_info "Caddyfile regenerated. Restart Caddy: make caddy restart"
    exit 0
fi
